...
namespace shard
{
	...
	public:
		...
		std::unique_ptr<UsingDirectiveSyntax> ReadUsingDirective(SourceProvider& reader, SyntaxNode* parent);
		std::unique_ptr<NamespaceDeclarationSyntax> ReadNamespaceDeclaration(SourceProvider& reader, SyntaxNode* parent);

		// 2. Type declarations
		std::unique_ptr<MemberDeclarationSyntax> ReadMemberDeclaration(SourceProvider& reader, SyntaxNode* parent);
		std::unique_ptr<ClassDeclarationSyntax> ReadClassDeclaration(SourceProvider& reader, MemberDeclarationInfo& info, SyntaxNode* parent);
		...
		std::vector<std::unique_ptr<AttributeSyntax>> ReadAttributeList(SourceProvider& reader, SyntaxNode* parent);
		void ReadTypeBody(SourceProvider& reader, TypeDeclarationSyntax* syntax);
		//TypeDeclarationSyntax* make_type(MemberDeclarationInfo& info, SyntaxNode* parent);
		...
		std::unique_ptr<IndexatorDeclarationSyntax> ReadIndexatorDeclaration(SourceProvider& reader, MemberDeclarationInfo& info, SyntaxNode* parent);
		...
		std::unique_ptr<KeywordStatementSyntax> ReadKeywordStatement(SourceProvider& reader, SyntaxNode* parent);
		std::unique_ptr<ReturnStatementSyntax> ReadReturnStatement(SourceProvider& reader, SyntaxNode* parent);
		std::unique_ptr<ThrowStatementSyntax> ReadThrowStatement(SourceProvider& reader, SyntaxNode* parent);
		std::unique_ptr<BreakStatementSyntax> ReadBreakStatement(SourceProvider& reader, SyntaxNode* parent);
		std::unique_ptr<ContinueStatementSyntax> ReadContinueStatement(SourceProvider& reader, SyntaxNode* parent);
		std::unique_ptr<DeferStatementSyntax> ReadDeferStatement(SourceProvider& reader, SyntaxNode* parent);

		// 6. Lexical structures
		std::unique_ptr<ConditionalClauseBaseSyntax> ReadConditionalClause(SourceProvider& reader, SyntaxNode* parent);
		std::unique_ptr<WhileStatementSyntax> ReadWhileStatement(SourceProvider& reader, SyntaxNode* parent);
		...
		std::unique_ptr<ArgumentsListSyntax> ReadArgumentsList(SourceProvider& reader, SyntaxNode* parent);
		std::unique_ptr<IndexatorListSyntax> ReadIndexatorList(SourceProvider& reader, SyntaxNode* parent);
		std::unique_ptr<ParametersListSyntax> ReadIndexerParametersList(SourceProvider& reader, SyntaxNode* parent);
		std::unique_ptr<ParametersListSyntax> ReadParametersList(SourceProvider& reader, SyntaxNode* parent);
